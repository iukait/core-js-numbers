...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const rootOfV1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const rootOfV2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(mult / (rootOfV1 * rootOfV2));
}
...
function getCountOfOddNumbers(number) {
  ...
  return arrOfNum.filter((i) => i % 2 !== 0).length;
}
